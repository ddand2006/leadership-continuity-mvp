...
import { generateCandidateCoachingNarrative } from "@/lib/candidate-coaching-narrative";
  ...
  try {
    if (!hasOpenAIEnv()) {
      throw new ApiRouteError(
        "Add OPENAI_API_KEY to .env.local before generating coaching narratives.",
        400,
      );
    }
    ...
    if (!isAdminAppRole(profile.role) && !mentorHasAccess) {
      throw new ApiRouteError(
        "You do not have access to generate coaching guidance for this candidate.",
        403,
      );
    }
    ...
    const topStrengths = strengthBuckets.primary
    ...
    const panelMap = new Map(
      (panelsResult.data ?? []).map((panel) => [panel.id, panel]),
    );
    const scoreHistory = (scoresResult.data ?? [])
      .filter((score) => score.competency_id === payload.competencyId)
      .map((score) => {
        const panel = panelMap.get(score.panel_id);

        return {
          panelName: panel?.panel_name ?? "Interview panel",
          dateCompleted: panel?.date_completed ?? null,
          scoreNumeric: Number(score.score_numeric),
          evidenceNotes: score.evidence_notes,
          concernNotes: score.concern_notes,
        };
      })
      .sort((left, right) => {
        const leftDate = left.dateCompleted ?? "";
        const rightDate = right.dateCompleted ?? "";

        if (leftDate !== rightDate) {
          return leftDate.localeCompare(rightDate);
        }

        return left.panelName.localeCompare(right.panelName);
      });

    const narrative = await generateCandidateCoachingNarrative({
      candidate: {
