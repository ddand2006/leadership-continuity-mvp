...
export async function POST(request: Request) {
  try {
    ...
    files.forEach((file) => {
      assertAcceptedFileType(file, ["pdf", "csv", "txt"]);
    });
    ...
    const uploadedDocuments = await Promise.all(
      files.map(async (file) => ({
        file,
        fileName: file.name,
        fileExtension: file.name.toLowerCase().split(".").at(-1) ?? "",
        fileSizeBytes: file.size,
        mimeType: file.type || null,
        text: await extractTextFromUploadedFile(file, ["pdf", "csv", "txt"]),
      })),
    );
    ...
    const analysis = await analyzeStrengthsDocuments({
      ...
      documents: [
        ...existingDocumentsToKeep.map((document) => ({
    ...
    return NextResponse.json({
      message: warningMessage
        ? `Added ${files.length} file${files.length === 1 ? "" : "s"}, analyzed ${existingDocumentsToKeep.length + uploadedDocuments.length} file${existingDocumentsToKeep.length + uploadedDocuments.length === 1 ? "" : "s"} on record, and imported ${analysis.rankings.length} strengths for ${candidateResult.data.full_name}. ${warningMessage}`
        : `Added ${files.length} file${files.length === 1 ? "" : "s"}, analyzed ${existingDocumentsToKeep.length + uploadedDocuments.length} file${existingDocumentsToKeep.length + uploadedDocuments.length === 1 ? "" : "s"} on record, and imported ${analysis.rankings.length} strengths for ${candidateResult.data.full_name}.`,
      count: analysis.rankings.length,
    });
  } catch (error) {
