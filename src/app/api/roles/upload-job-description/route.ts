...
export async function POST(request: Request) {
  try {
    ...
    const upload = await admin.storage.from(bucket).upload(path, Buffer.from(await file.arrayBuffer()), { contentType: file.type || "application/octet-stream", upsert: false });
    ...
    if (saved.error) { await admin.storage.from(bucket).remove([path]); throw new ApiRouteError(saved.error.message, 500); }
