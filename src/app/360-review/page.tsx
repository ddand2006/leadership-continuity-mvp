...
export default async function Review360Page() {
  ...
  return <main className="app-page"><div className="mx-auto flex w-full max-w-[1380px] flex-col gap-8 px-6 py-12 sm:px-10 lg:px-12"><section className="theme-panel-strong rounded-[2rem] p-8"><p className="text-sm font-semibold tracking-[.16em] text-teal-700 uppercase">Current-role feedback</p><h1 className="mt-3 font-display text-5xl text-slate-900">360° Role Competency Review</h1><p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">Evaluate observable behavior against the employee’s current Ideal Role Composite. Results are developmental, confidential, and distinct from succession readiness.</p></section><Review360Workspace assignments={(assignmentsResult.data ?? []).map(a=>({ id:a.id, employeeName:users.get(a.organization_user_id) ?? "Unknown employee", roleTitle:roles.get(a.role_id) ?? "Unknown role", department:a.department }))} employees={(usersResult.data??[]).map(user=>({id:user.id,name:`${user.first_name} ${user.last_name}`}))} roles={(rolesResult.data??[]).map(role=>({id:role.id,title:role.title}))} cycles={(cyclesResult.data ?? []).map(c=>{ const related=respondents.filter(r=>r.review_cycle_id===c.id); return { id:c.id,title:c.title,employeeName:users.get(c.employee_organization_user_id) ?? "Unknown employee",roleTitle:c.role_title,status:c.status,dueDate:c.due_date,invited:related.length,completed:related.filter(r=>r.status==="completed").length }; })}/></div></main>;
}
