...
export function AffiliateLanding({ branding: b, slug, preview = false }: { branding: AffiliateBranding; slug: string; preview?: boolean }) {
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className="mx-auto max-w-5xl px-6 py-12">
      ...
      <footer className="mt-16 border-t pt-6 text-sm leading-7 text-slate-600"><p>Presented by {b.displayName}. Software and subscription services provided by Leadership Continuity.</p><p>This partner may receive compensation for your subscription.</p><div className="mt-3 flex flex-col gap-1">{b.contactName && <p>{b.contactName}</p>}{b.address && <address className="whitespace-pre-line not-italic">{b.address}</address>}<div className="flex flex-wrap gap-x-4 gap-y-1"><a className="underline" href={`mailto:${b.contactEmail}`}>Email {b.contactName || b.displayName}</a>{b.phone && <a className="underline" href={`tel:${b.phone}`}>{b.phone}</a>}</div></div></footer>
    </div>
  </main>;
}
