...
export async function CoachingWorkspace({ path, filters }: {
    ...
    if(isScalePath(path)) content=await ScaleWorkspace({path,filters:Object.fromEntries(Object.entries(filters).filter((entry):entry is [string,string]=>entry[1]!==undefined))});
        ...
        notFound();
    return <main className="app-page"><div className="mx-auto flex max-w-[1380px] flex-col gap-6 px-5 py-10 sm:px-10"><header><p className="text-sm font-semibold uppercase tracking-widest text-teal-700">Leader Continuity</p><h1 className="mt-2 text-3xl font-semibold text-teal-950">Leadership Coaching</h1><p className="mt-2 text-slate-600">Connect leadership development, professional coaching, and shared progress.</p></header><nav aria-label="Coaching" className="flex flex-wrap gap-4 rounded-2xl bg-white p-4 text-sm font-semibold">{[['/coaching', 'Overview'], ['/coaching/marketplace', 'Marketplace'], ['/coaching/engagements', 'Engagements'], ['/coaching/my', 'My Coaching'], ['/coaching/requests','Requests'], ['/coaching/notifications','Notifications'],['/coaching/feedback','Feedback'], ...(ctx.admin?[['/coaching/analytics','Analytics'],['/coaching/programs','Programs']]:[]), ...(ctx.coach?[['/coaching/quality','Quality'],['/coaching/onboarding','Marketplace Readiness']]:[]), ...(ctx.admin||ctx.coach?[['/coaching/outcomes','Development Outcomes']]:[]), ...(ctx.admin ? [['/coaching/billing','Billing']] : []), ...(ctx.coach ? [['/coaching/earnings','Earnings'],['/coaching/coach', 'Coach Dashboard'], ['/coaching/opportunities','Opportunities'], ['/coaching/availability','Availability'], ['/coaching/clients', 'Clients'], ['/coaching/sessions', 'Sessions'], ['/coaching/log', 'Coaching Log'], ['/coaching/profile', 'Profile']] : []), ...(ctx.platformAdmin ? [['/admin/coaching/marketplace','Marketplace Health'],['/admin/coaching/coach-applications','Coach Applications'],['/admin/coaching/development-rules','Development Rules'],['/admin/coaching/financials','Financials'], ['/admin/coaching/packages','Packages'],['/admin/coaching','Coaching Operations'], ['/admin/coaching/coaches', 'Coach Administration'], ['/admin/coaching/requests', 'Access Requests']] : [])].map(([href, label]) => <Link className="text-teal-900 underline underline-offset-4" key={href} href={href}>{label}</Link>)}</nav>{content}</div></main>;
}
