...
export function AuthForms(props: { initialMode?: "signin" | "signup"; affiliateSlug?: string }) {
  ...
  async function handleSignUp(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error) throw error;
      if (!data.session) {
        setSuccessMessage("Check your email to confirm your account. You will choose your plan after confirmation.");
        return;
      }
      await syncSessionToServer(data.session);
      ...
      window.location.assign("/subscribe?setup=1");
