        ...
        {assignments.map((assignment) => {
          ...
          return (
            <Link
              ...
              className={`rounded-2xl border px-4 py-3 text-left transition ${isSelected ? "interactive-contrast border-slate-900 bg-slate-900 text-white shadow-[0_14px_30px_rgba(15,23,42,0.14)]" : "border-slate-200 bg-white text-slate-800 hover:border-slate-300 hover:bg-slate-50"}`}
            >
              <span
              ...
              <span className={`mt-1 block text-xs ${isSelected ? "text-white" : "text-slate-500"}`}>{assignment.roleTitle}</span>
              <span className={`mt-1 block text-xs ${isSelected ? "text-white" : "text-slate-500"}`}>Mentor: {assignment.mentorName}</span>
              <span className={`mt-1 block text-xs ${isSelected ? "text-white" : "text-slate-500"}`}>{assignment.currentTitle ?? "No current title"} · {(assignment.status ?? "active").replaceAll("_", " ")}</span>
            </Link>
          );
        })}
